...
describe('zhconv', () => {
  it('converts simplified characters to traditional', async () => {
    expect(await toTraditional('乐器行')).toBe('樂器行');
    expect(await toTraditional('钢琴')).toBe('鋼琴');
  });

  it('leaves already-traditional text unchanged', async () => {
    expect(await toTraditional('鋼琴')).toBe('鋼琴');
  });

  it('does not translate vocabulary, only characters (matches zhconv.py scope)', async () => {
    ...
    expect(await toTraditional('鼠标')).toBe('鼠標');
  });
