...
## 注音發音速查
當你看到已揭露注音時，用下表確認每個符號的發音（不用記，參考用）：
...
結合韻：ㄧ=y/i  ㄨ=w/u  ㄩ=yü/ü（例：ㄩㄥ=iong）

聲調：ˉ(一聲) ˊ(二聲) ˇ(三聲) ˋ(四聲) ˙(輕聲)

**容易搞混的：ㄙ=s(思/四) ≠ ㄕ=sh(書/樹)　ㄗ=z(資) ≠ ㄓ=zh(知)　ㄘ=c(雌) ≠ ㄔ=ch(吃)**
...
|---|---|---|
| ㄖ | 熱血(ㄖㄜˋ)、銳利(ㄖㄨㄟˋ)、柔軟(ㄖㄡˊ) | 單字（聲調揭露後不可能只看到聲母） |
| ㄙ | 森林(ㄙㄣ)、速度(ㄙㄨˋ)、四(ㄙˋ) | 書(ㄕㄨ，ㄕ不是ㄙ) |
| ㄊㄡˊ | 頭盔(ㄊㄡˊ)、投籃(ㄊㄡˊ)、頭痛(ㄊㄡˊ) | 投（一整格音節已揭露，後面應還有字） |
| ㄨㄛ | 渦輪(ㄨㄛ)、握手(ㄨㄛˋ)、我愛(ㄨㄛˇ) | 僅一個字（ㄨㄛ是完整音節但通常不是單字） |
